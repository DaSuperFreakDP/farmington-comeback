...
def get_match_stats_html(username):
    ...
    # Use index-based matchday numbers (1-indexed) instead of stored values to avoid duplicates
    cumulative_injuries_by_day = {}
    cumulative_injury_points_by_day = {}
    running_injuries = {}
    running_injury_points = {}

    for idx, entry in enumerate(stats_data["data"]):
        matchday = idx + 1  # Use 1-indexed position as matchday number
    ...
    # Render matchdays in reverse order (newest first) with correct numbering
    total_matchdays = len(stats_data["data"])
    for idx, entry in enumerate(reversed(stats_data["data"])):
        # Calculate correct matchday number: total - reverse_index = actual position
        matchday = total_matchdays - idx
        season = entry["season"]
        ...
        match_total = 0
        catastrophe_type = entry.get("catastrophe_type", 0)
        ...
        for farmer in farmers:
            ...
            # Determine catastrophe loss display
            cat_display = ""
            if catastrophe_type == 1:
                # Type 1: Only affected farmer loses points
                if affected == name and catlost > 0:
                    cat_display = f"{catlost} ABC"
            elif catastrophe_type == 2:
                # Type 2: All farmers lose points
                if catlost > 0:
                    cat_display = f"{catlost} all"
            elif catastrophe_type == 3:
                # Type 3: All farmers lose all points
                if catlost > 0:
                    cat_display = f"{catlost} all"
            ...
            html += f"<tr><td>{name}{heart_emoji}</td><td>{job}</td><td>{total_pts}</td><td>{task_pts}</td><td>{crop_pts}</td><td>{cat_display}</td><td>{injlost}</td><td>{injtot}</td><td>{injptot}</td><td>{avg:.2f}</td></tr>"
