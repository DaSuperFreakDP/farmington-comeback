...
from stats import get_user_stats, update_user_stats, get_match_stats_html, load_stats, save_stats
...
# Lock to prevent duplicate matchday processing
_matchday_lock = threading.Lock()
_last_matchday_time = 0
...
def get_league_matchday_sync_status(league_code):
    """Check if all players in a league have the same number of matchday entries"""
    leagues = load_leagues()
    ...
    for player in league["players"]:
        user_data = get_user_stats(player)
        matchday_counts[player] = len(user_data.get("data", []))
    
    min_count = min(matchday_counts.values()) if matchday_counts else 0
    max_count = max(matchday_counts.values()) if matchday_counts else 0
    
    return {
        "min_matchdays": min_count,
        "max_matchdays": max_count,
        "player_counts": matchday_counts,
        "is_synced": min_count == max_count
    }, None

def sync_league_matchdays(league_code):
    """Ensure all players in a league have the same number of matchday entries"""
    leagues = load_leagues()
    league = leagues.get(league_code)
    if not league:
        return False, "League not found"
    
    status, error = get_league_matchday_sync_status(league_code)
    if error:
        return False, error
    
    if status["is_synced"]:
        return True, "All players already synced"
    
    # Get the minimum number of matchdays (only count completed matchdays everyone has)
    min_matchdays = status["min_matchdays"]
    
    # Trim any extra matchdays from players who are ahead
    synced_count = 0
    for player in league["players"]:
        user_data = get_user_stats(player)
        current_data = user_data.get("data", [])
        
        if len(current_data) > min_matchdays:
            # Trim to the minimum
            user_data["data"] = current_data[:min_matchdays]
            update_user_stats(player, user_data)
            synced_count += 1
            logging.info(f"Synced {player}: trimmed from {len(current_data)} to {min_matchdays} matchdays")
    
    return True, f"Synced {synced_count} players to {min_matchdays} matchdays"

def run_automated_matchday():
    """Run matchday for all users with complete teams"""
    global _last_matchday_time
    
    # Prevent duplicate execution within 100 seconds (leaving buffer for 120 second interval)
    with _matchday_lock:
        current_time = time.time()
        if current_time - _last_matchday_time < 100:
            logging.debug(f"Skipping duplicate matchday execution (last run {current_time - _last_matchday_time:.1f}s ago)")
            return
        _last_matchday_time = current_time
    ...
    try:
        ...
        for league_code, league in active_leagues.items():
            ...
            # Sync all players in this league before running matchdays
            sync_success, sync_msg = sync_league_matchdays(league_code)
            if sync_success and sync_msg != "All players already synced":
                logging.info(f"League {league_code} sync result: {sync_msg}")
            
            # Get current sync status to only process players at the minimum
            status, _ = get_league_matchday_sync_status(league_code)
            min_matchdays = status["min_matchdays"] if status else 0
            ...
            for username in league["players"]:
                ...
                # Only run matchday if this player is at the minimum matchday count
                # This prevents players from getting ahead
                current_matchday_count = len(user_data.get("data", []))
                
                if drafted_team and has_complete_team and current_matchday_count == min_matchdays:
                    try:
                        ...
                        logging.info(f"Completed matchday for {username} at cycle {global_matchday}")
...
def profile():
    ...
    try:
        ...
        # Combine user_info and user_data into user_profile for the template
        user_profile = {
            "profile_pic": user_info.get("profile_pic"),
            "team_name": user_data.get("team_name", ""),
            "team_chant": user_data.get("team_chant", "")
        }
...
def run_matchday():
    ...
    try:
        # First, ensure all players are synced to the same number of matchdays
        sync_success, sync_msg = sync_league_matchdays(current_league["code"])
        if sync_success and sync_msg != "All players already synced":
            logging.info(f"League sync result: {sync_msg}")
        
        # Get current sync status to only process players at the minimum
        status, _ = get_league_matchday_sync_status(current_league["code"])
        min_matchdays = status["min_matchdays"] if status else 0
...
def market():
    ...
    for farmer in league_farmer_pool:
        if farmer["name"] not in drafted_farmers:
            ...
            # Load crop preferences for this farmer
            crop_preferences = {}
            try:
                with open("farmer_crop_preferences.json", "r") as f:
                    all_preferences = json.load(f)
                    crop_preferences = all_preferences.get(farmer["name"], {})
            except FileNotFoundError:
                pass
